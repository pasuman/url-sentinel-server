...
[ Sentinel Server (Spring Boot) ]
   ...
   ├─ Decision Engine      → Verdict aggregation
...
- **9 rules**: AiModel, UrlLength, ExcessiveSpecialChars, SuspiciousKeyword, IpAddressDomain, UrlShortener, ExcessiveSubdomains, SuspiciousTld, EncodedCharacters

**Decision Engine** (`com.seheon99.urlsentinel.decision`):
- `DefaultDecisionEngine` → aggregates phishing rule results into ALLOW/REJECT
...
**POST /analyze** - URL phishing detection endpoint:
- **Request**: Requires `url` (string)
- **Response**: Includes `verdict` (ALLOW/REJECT), `reasons` (list of triggered rules), `riskScore` (0-100)
- **Simplified**: Direct URL analysis without SSL verification step
